...
def _is_mps_tensor(x):
    return hasattr(x, "is_mps") and x.is_mps


def _maybe_to_cpu(x):
    if _is_mps_tensor(x):
        return x.detach().cpu(), True
    return x, False
...
def handle_torch_Index(the_class):
    def torch_replacement_add(self, x, numeric_type=faiss.Float32):
        ...
        x_cpu, is_mps = _maybe_to_cpu(x)
        n, d = x_cpu.shape
        assert d == self.d
        if numeric_type == faiss.Float32:
            x_ptr = swig_ptr_from_FloatTensor(x_cpu)
        elif numeric_type == faiss.Float16:
            x_ptr = swig_ptr_from_HalfTensor(x_cpu)
        else:
            raise ValueError(
        ...
        if x.is_cuda:
            ...
            assert hasattr(self, 'getDevice'), \
                'MPS tensor on CPU index not allowed'
            with using_mps():
                self.add_ex(n, x_ptr, numeric_type)
        else:
            # CPU torch
    ...
    def torch_replacement_add_with_ids(self, x, ids,
        ...
        ids_cpu, ids_is_mps = _maybe_to_cpu(ids)
        ids_ptr = swig_ptr_from_IndicesTensor(ids_cpu)
    ...
    def torch_replacement_assign(self, x, k, labels=None):
        ...
        if labels is None:
            labels = torch.empty(n, k, device='cpu', dtype=torch.int64) if is_mps \
                else torch.empty(n, k, device=x.device, dtype=torch.int64)
        else:
            assert type(labels) is torch.Tensor
            assert labels.shape == (n, k)
        labels_cpu, labels_is_mps = _maybe_to_cpu(labels)
        L_ptr = swig_ptr_from_IndicesTensor(labels_cpu)
    ...
    def search_methods_common(x, k, D, I, numeric_type=faiss.Float32):
        x_cpu, x_is_mps = _maybe_to_cpu(x)
        n, d = x_cpu.shape
        if numeric_type == faiss.Float32:
            x_ptr = swig_ptr_from_FloatTensor(x_cpu)
        elif numeric_type == faiss.Float16:
        ...
        if D is None:
            if x_is_mps:
                ...
                D_cpu = torch.empty(n, k, device='cpu', dtype=torch.float32)
                D_is_mps = True
            else:
                D = torch.empty(n, k, device=x.device, dtype=torch.float32)
                D_cpu, D_is_mps = _maybe_to_cpu(D)
        else:
            assert type(D) is torch.Tensor
            assert D.shape == (n, k)
            D_cpu, D_is_mps = _maybe_to_cpu(D)
        D_ptr = swig_ptr_from_FloatTensor(D_cpu)

        if I is None:
            if x_is_mps:
                I = torch.empty(n, k, device=x.device, dtype=torch.int64)
                I_cpu = torch.empty(n, k, device='cpu', dtype=torch.int64)
                I_is_mps = True
            else:
                I = torch.empty(n, k, device=x.device, dtype=torch.int64)
                I_cpu, I_is_mps = _maybe_to_cpu(I)
        else:
            assert type(I) is torch.Tensor
            assert I.shape == (n, k)
            I_cpu, I_is_mps = _maybe_to_cpu(I)
        I_ptr = swig_ptr_from_IndicesTensor(I_cpu)

        return (
            x_cpu, D_cpu, I_cpu, x_ptr, D_ptr, I_ptr, D, I,
            x_is_mps or D_is_mps or I_is_mps)
    ...
    def torch_replacement_search(self, x, k, D=None, I=None,
        ...
        x_cpu, D_cpu, I_cpu, x_ptr, D_ptr, I_ptr, D, I, needs_mps_copy = \
            search_methods_common(x, k, D, I)
        ...
        if needs_mps_copy:
            if D.device.type == 'mps':
                D.copy_(D_cpu.to(D.device))
            if I.device.type == 'mps':
                I.copy_(I_cpu.to(I.device))
        return D, I
    ...
    def torch_replacement_search_and_reconstruct(self, x, k, D=None, I=None,
        ...
        if R is None:
            R = torch.empty(
                n, k, d,
                device='cpu' if needs_mps_copy else x.device,
                dtype=torch.float32)
        else:
            assert type(R) is torch.Tensor
            assert R.shape == (n, k, d)
        R_cpu, R_is_mps = _maybe_to_cpu(R)
        R_ptr = swig_ptr_from_FloatTensor(R_cpu)
        ...
        elif needs_mps_copy or R_is_mps:
            assert hasattr(self, 'getDevice'), \
                'MPS tensor on CPU index not allowed'
            with using_mps():
                self.search_and_reconstruct_c(n, x_ptr, k, D_ptr, I_ptr, R_ptr)
        ...
        if needs_mps_copy or R_is_mps:
            if D.device.type == 'mps':
                D.copy_(D_cpu.to(D.device))
            if I.device.type == 'mps':
                I.copy_(I_cpu.to(I.device))
            if R.device.type == 'mps':
                R.copy_(R_cpu.to(R.device))
        return D, I, R
    ...
    def torch_replacement_search_preassigned(self, x, k, Iq, Dq, *,
        ...
        Iq = Iq.contiguous()
        Iq_cpu, Iq_is_mps = _maybe_to_cpu(Iq)
        Iq_ptr = swig_ptr_from_IndicesTensor(Iq_cpu)

        if Dq is not None:
            Dq = Dq.contiguous()
            assert Dq.shape == Iq.shape
            Dq_cpu, Dq_is_mps = _maybe_to_cpu(Dq)
            Dq_ptr = swig_ptr_from_FloatTensor(Dq_cpu)
        else:
            Dq_is_mps = False
            Dq_ptr = None
        ...
        if x.is_cuda:
            ...
            with using_stream(self.getResources()):
                self.search_preassigned_c(n, x_ptr, k, Iq_ptr, Dq_ptr,
                                          D_ptr, I_ptr, False)
        elif needs_mps_copy or x_is_mps or Iq_is_mps or Dq_is_mps:
            assert hasattr(self, 'getDevice'), \
                'MPS tensor on CPU index not allowed'
            with using_mps():
                self.search_preassigned_c(n, x_ptr, k, Iq_ptr, Dq_ptr,
    ...
    def torch_replacement_update_vectors(self, keys, x):
        ...
        (n, ) = keys.shape
        keys_cpu, keys_is_mps = _maybe_to_cpu(keys)
        keys_ptr = swig_ptr_from_IndicesTensor(keys_cpu)
    ...
    def torch_replacement_range_search(self, x, thresh):
        ...
        assert not x.is_cuda and not x_is_mps, \
    ...
    def torch_replacement_sa_decode(self, codes, x=None):
        ...
        codes_cpu, codes_is_mps = _maybe_to_cpu(codes)
        n, cs = codes_cpu.shape
        assert cs == self.sa_code_size()
        codes_ptr = swig_ptr_from_UInt8Tensor(codes_cpu)

        if x is None:
            if codes_is_mps:
                x = torch.empty(n, self.d, dtype=torch.float32, device=codes.device)
                x_cpu = torch.empty(n, self.d, dtype=torch.float32, device='cpu')
                x_is_mps = True
            else:
                x = torch.empty(n, self.d, dtype=torch.float32, device='cpu')
                x_cpu = x
                x_is_mps = False
        else:
            assert type(x) is torch.Tensor
...
def torch_replacement_knn(xq, xb, k, metric=faiss.METRIC_L2, metric_arg=0):
    ...
    assert not xb.is_cuda and not _is_mps_tensor(xb), \
        "use knn_gpu for GPU tensors"

    nq, d2 = xq.size()
    assert d2 == d
    assert xq.is_contiguous()
    assert xq.dtype == torch.float32
    assert not xq.is_cuda and not _is_mps_tensor(xq), \
        "use knn_gpu for GPU tensors"

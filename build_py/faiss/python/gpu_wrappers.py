...
_HAS_GPU_CLONER = 'index_cpu_to_gpu_multiple' in globals()
...
def index_cpu_to_gpu_multiple_py(resources, index, co=None, gpus=None):
    ...
    if not _HAS_GPU_CLONER:
        if len(resources) != 1:
            raise RuntimeError("MPS backend supports a single device")
        res = resources[0]
        if isinstance(index, IndexBinary):
            return GpuIndexBinaryFlat(res, index)
        if isinstance(index, IndexIVFPQ):
            return GpuIndexIVFPQ(res, index)
        if isinstance(index, IndexIVFFlat):
            return GpuIndexIVFFlat(res, index)
        if isinstance(index, IndexFlat):
            return GpuIndexFlat(res, index)
        raise RuntimeError(
            "MPS backend supports IndexFlat, IndexBinaryFlat, IndexIVFFlat, IndexIVFPQ"
        )
